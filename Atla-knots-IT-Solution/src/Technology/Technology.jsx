...
// // Import Engagement Thunks (Add these in your technologySlice.js)
// import {
//   incrementPostView,
//   togglePostLike,
//   sendCommentOtp,
...
//       dispatch(togglePostLike(itemId));
...
//     await dispatch(sendCommentOtp({ postId: pendingItemId, name: userInfo.name, email: userInfo.email }));
//     setStep("otp");
...
//     setLoadingAction(true);
//     const result = await dispatch(postCommentWithOtp({
...
//                         <div className="flex items-center gap-1"><Eye className="w-4 h-4" /><span>{news.views || 0}</span></div>
//                       </div>
...
//                             {(news.comments?.length || 0) + (updatedItems[itemId]?.comments?.length || 0)}
//                           </button>
//                         </div>
//                         <button
//                           onClick={() => handleAction("view", itemId)}
...
//               <button onClick={() => setShowCommentModal(false)} className={`flex-1 py-3 rounded-xl font-medium border ${isDark ? "border-gray-700 hover:bg-gray-800" : "border-gray-300 hover:bg-gray-100"}`}>Cancel</button>
//               <button onClick={submitComment} disabled={loadingAction || !commentText.trim()} className={`flex-1 py-3 rounded-xl font-medium text-white flex items-center justify-center ${commentText.trim() ? "bg-red-600 hover:bg-red-700" : "bg-gray-400 cursor-not-allowed"}`}>
//                 {loadingAction ? <Loader2 className="w-5 h-5 animate-spin" /> : "Post Comment"}
//               </button>
//             </div>
//           </div>
//         </div>
...
//                 <input type="tel" placeholder="Phone Number (Optional)" value={userInfo.phone} onChange={(e) => setUserInfo({ ...userInfo, phone: e.target.value })} className={`w-full p-3 rounded-lg mb-6 border ${isDark ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-300"}`} />
//                 <button onClick={sendOtp} disabled={loadingAction} className="w-full bg-red-600 hover:bg-red-700 py-3 rounded-lg text-white font-medium disabled:opacity-70">Send OTP</button>
//               </>
//             ) : (
//               <>
